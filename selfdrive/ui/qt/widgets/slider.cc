...
void CustomSlider::initialize() {
  ...
  Params().put(param.toStdString(), std::to_string(defaultVal));
  try {
    double value = std::stod(params.get(param.toStdString())); // Get the value from the param. It could be float so its casted to string and then to double and QSlider only accepts int
  ...
  connect(this, &CustomSlider::sliderReleasedWithValue, [=](int value) {
    ...
    double dValue = paramMin + (paramMax - paramMin) * (value - sliderMin) / (sliderMax - sliderMin);
    params.put(param.toStdString(), std::to_string(dValue));
  });
